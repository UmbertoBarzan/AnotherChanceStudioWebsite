...
const Navigation = () => {
  ...
  return (
    <nav className="mt-px font-display text-5xl font-medium tracking-tight text-white">
      <NavigationRow>
        <NavigationItem href="/#projects">{t.featuredProjects.eyebrow}</NavigationItem>
        <NavigationItem href="/#about">{t.nav.about}</NavigationItem>
      </NavigationRow>
      <NavigationRow>
        <NavigationItem href="/#services">{t.services.eyebrow}</NavigationItem>
...
const RootLayoutInner = ({ children, pathName }) => {
  const { language } = useLanguage();
  const t = translations[language] || translations.en;
  ...
  return (
    <MotionConfig transition={shouldReduceMotion ? { duration: 0 } : undefined}>
      ...
      <motion.div
        ...
        className={clsx(
          "relative flex flex-auto overflow-hidden bg-white",
          isHome ? "pt-0" : "pt-14"
        )}
      >
        <motion.div
...
const RootLayout = ({ children }) => {
  ...
  return (
    <RootLayoutInner key={pathName} pathName={pathName}>
      {children}
    </RootLayoutInner>
  );
};
